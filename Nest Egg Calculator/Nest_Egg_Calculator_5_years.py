...
def montecarlo(returns):
    ...
    while case_count < int(num_cases):
        ...
        for year, lifespan_return in enumerate(lifespan_returns):
            ...
            if year == 0:
                withdrawal_first_five_years_adjusted = int(withdrawal_first_five_years);
                withrawal_remainder_adjusted = int(withdrawal_remainder);
            else:
                withdrawal_first_five_years_adjusted = int(withdrawal_first_five_years_adjusted * (1 + inflation));
                withrawal_remainder_adjusted = int(withrawal_remainder_adjusted * (1 + inflation));

            #decide withdrawal based on year
            if year < 5:
                withdrawal_adjusted = withdrawal_first_five_years_adjusted;
            else:
                withdrawal_adjusted = withrawal_remainder_adjusted;
...
def bankrupt_probability(outcome, bankrupt_count):
    ...
    print('\nAnnual withdrawal in the first 5 years: ${:,}\n'.format(int(withdrawal_first_five_years)));
    print('\nAnnual withdrawal after 5 years: ${:,}\n'.format(int(withdrawal_remainder)));
...
withdrawal_first_five_years = default_input("Input annual pre-tax withdrawal for first five years (today's $): \n", '80000');
#validate the withdrawal value
while not withdrawal_first_five_years.isdigit():
    withdrawal_first_five_years = input("Invalid input! Enter an integer: \n");

withdrawal_remainder = default_input("Input annual pre-tax withdrawal for remainder (today's $): \n", '80000');
#validate the withdrawal value
while not withdrawal_remainder.isdigit():
    withdrawal_remainder = input("Invalid input! Enter an integer: \n");
